...
def generate_images_and_metadata(
    sets_to_visualize, model, max_events_per_set, max_images_per_set, save_path
):
    metadata_list = []
    ...
    for set_name in tqdm(sets_to_visualize):
        ...
        for file_idx, data_file in tqdm(
            enumerate(
                glob.glob(
                    os.path.join(NPZ_LOC / SET_UNDER_INVESTIGATION / set_name, "*.npz")
                )
            ),
            leave=False,
            desc=set_name,
            ...
            feats, fractional_energy, total_cell_energy = load_data_from_npz(
                filename_npz
            )

            filtered_features = feats[TRAIN_INPUTS]

            unstructured_filtered_features = rfn.structured_to_unstructured(
                filtered_features
            )
            ...
            for window_index, window in enumerate(feats):
                if (events_in_data >= max_events_per_set) and (
                    max_events_per_set != -1
                ):
                    end_set = True
                    break
                events_in_data += 1
                ...
                for point_index, point in enumerate(window):
                    ...
                    if point["category"] == POINT_TYPE_ENCODING["focus hit"]:
                        ...
                        cell_model_attribution.append(
                            model_results[window_index][point_index]
                        )
                        cell_truth_attribution.append(
                            fractional_energy[window_index][point_index]
                        )
                    elif point["category"] == POINT_TYPE_ENCODING["unfocus hit"]:
                        if point["track_ID"] in non_focus_tracks:
                            non_focus_tracks[point["track_ID"]][
                                "non_focus_hit_x"
                            ].append(point["normalized_x"])
                            non_focus_tracks[point["track_ID"]][
                                "non_focus_hit_y"
                            ].append(point["normalized_y"])
                            non_focus_tracks[point["track_ID"]][
                                "non_focus_hit_z"
                            ].append(point["normalized_z"])
                        else:
                            non_focus_tracks[point["track_ID"]] = {
                ...
                cell_truth_attribution_cat = np.argmax(
                    rfn.structured_to_unstructured(cell_truth_attribution), axis=-1
                )
                ...
                accuracy = np.count_nonzero(
                    cell_model_attribution_cat == cell_truth_attribution_cat
                )

                activated_energy = np.sum(
                    window["cell_E"][valid_cell_mask][truth_focal_mask]
                )
                ideal_activation_energy = np.sum(
                    window["cell_E"][valid_cell_mask][truth_focal_mask]
                )
                total_truth_track_energy = np.sum(
                    window["truth_cell_total_energy"][valid_cell_mask]
                    * window["truth_cell_focal_fraction_energy"][valid_cell_mask]
                )
                ...
                if (
                    RENDER_IMAGES
                    and (
                        max_images_per_set == -1 or images_rendered < max_images_per_set
                    )
                ) or nCells > 1000:
                    images_rendered += 1
                    # convert to plot:
                    ...
                    fig.suptitle(
                        f'Set: {set_name}, Event: {window[0]["event_number"]} Track: {window[0]["track_ID"]}, $\\sum E={sum(cell_total_energy):.2f}$, nCells={len(cell_x_list)}, pt={window["track_pt"][0]:.2f}, Activated Energy: {float(activated_energy):.4f}, Ideal Activation Energy: {ideal_activation_energy:.4f} GeV, Truth Track E: {total_truth_track_energy:.5f} GeV'
                    )
                    ...
                    ax1.set_title(
                        f"Model Prediction - {nmodel_focal} focal | {nmodel_non_focal} non-focal | {nmodel_neutral} neutral"
                    )
                    ax1.scatter(
                        cell_x_array_np[model_focal_mask],
                        cell_y_array_np[model_focal_mask],
                        cell_z_array_np[model_focal_mask],
                        label="Focal Hits",
                        c="green",
                    )
                    ax1.scatter(
                        cell_x_array_np[model_non_focal_mask],
                        cell_y_array_np[model_non_focal_mask],
                        cell_z_array_np[model_non_focal_mask],
                        label="Non Focal Hits",
                        c="yellow",
                    )
                    ax1.scatter(
                        cell_x_array_np[model_neutral_mask],
                        cell_y_array_np[model_neutral_mask],
                        cell_z_array_np[model_neutral_mask],
                        label="Neutral Hits",
                        c="red",
                    )

                    ax2.set_title(
                        f"Truth Values - {ntruth_focal} focal | {ntruth_non_focal} non-focal | {ntruth_neutral} neutral"
                    )
                    ax2.scatter(
                        cell_x_array_np[truth_focal_mask],
                        cell_y_array_np[truth_focal_mask],
                        cell_z_array_np[truth_focal_mask],
                        label="Focal Hits",
                        c="green",
                    )
                    ax2.scatter(
                        cell_x_array_np[truth_non_focal_mask],
                        cell_y_array_np[truth_non_focal_mask],
                        cell_z_array_np[truth_non_focal_mask],
                        label="Non Focal Hits",
                        c="yellow",
                    )
                    ax2.scatter(
                        cell_x_array_np[truth_neutral_mask],
                        cell_y_array_np[truth_neutral_mask],
                        cell_z_array_np[truth_neutral_mask],
                        label="Neutral Hits",
                        c="red",
                    )
                    ...
                    if np.any(mask):
                        sc3 = ax3.scatter(
                            cell_x_array_np[mask],
                            cell_y_array_np[mask],
                            cell_z_array_np[mask],
                            c=cell_total_energy_array[mask],
                            cmap="jet",
                            norm=LogNorm(),
                        )
                    else:
                    ...
                    plt.legend()
                    print(
                        f"saving: event={window[0]['event_number']:09}_track={window[0]['track_ID']:03}"
                    )
                    SET_IMAGE_PATH = TRACK_IMAGE_PATH / set_name
                    SET_IMAGE_PATH.mkdir(exist_ok=True)
                    plt.savefig(
                        SET_IMAGE_PATH
                        / f"event={window[0]['event_number']:09}_track={window[0]['track_ID']:03}.png"
                    )
                    plt.close()
