...
replay_model_path = Path(
    "/home/jhimmens/workspace/jetpointnet/models/delta/progressive_training/PointNet_best_name=autumn-pyramid-836.keras"
)
tune_model_path = Path(
    "/home/jhimmens/workspace/jetpointnet/models/rho/progressive_training/PointNet_best_name=dark-music-823.keras"
)
...
def progressive_data_generator(
    data_dir,
    simple_sets: list,
    complex_sets: list,
    batch_size: int,
    ...
    **kwargs,
    ...
    simple_genorator = consistant_data_generator(
        data_dir,
        {set_name: percent_simple_data / len(simple_sets) for set_name in simple_sets},
        int(batch_size * percent_simple_data),
        **kwargs,
    )
    complex_genorator = consistant_data_generator(
        data_dir,
        {
            set_name: (1 - percent_simple_data) / len(complex_sets)
            for set_name in complex_sets
        },
    ...
    while True:
        ...
        if simple_feats.size == 0:
            total_feats, total_targets, total_e_weights = (
                complex_feats,
                complex_targets,
                complex_e_weights,
            )
            ...
            total_e_weights = np.concatenate(
                [simple_e_weights, complex_e_weights], axis=0
            )
...
def train(experimental_configuration: dict = None):
    ...
    with wandb.init(
        ...
        for epoch in range(config.EPOCHS):
            ...
            if config.REPLAY:
                _train_generator = progressive_data_generator(
                    NPZ_SAVE_LOC(TRAIN) / "train",
                    SIMPLE_SETS,
                    COMPLEX_SETS,
                    config.BATCH_SIZE,
                    epoch,
                ...
                _train_generator = consistant_data_generator(
                    NPZ_SAVE_LOC(TRAIN) / "train", config.INPUT_SETS, config.BATCH_SIZE
                )
            ...
            if config.REPLAY:
                _val_generator = progressive_data_generator(
                    NPZ_SAVE_LOC(TRAIN) / "val",
                    SIMPLE_SETS,
                    COMPLEX_SETS,
                    config.BATCH_SIZE,
                    epoch,
                ...
                _val_generator = consistant_data_generator(
                    NPZ_SAVE_LOC(TRAIN) / "val", config.INPUT_SETS, config.BATCH_SIZE
                )
            ...
            wandb.log(
                {
                    ...
                    "percent_simple_data": (
                        max(
                            1 - epoch * config.REPLAY_LINEAR_DECAY_RATE,
                            config.REPLAY_MIN_FREQ,
                        )
                        if config.REPLAY
                        else 1
