...
def process_and_filter_cells(
    ...
    # TODO use hitsE_EM for truncating so that it matches Jessica's code
    ...
    cell_part_truth_Idxs_with_multiples = ak.flatten(
        event["cluster_cell_hitsTruthIndex"]
    )
    cell_part_truth_Es_with_multiples = ak.flatten(event["cluster_cell_hitsTruthE"])
    cell_hitsTruthTotalE_with_multiples = ak.flatten(
        event["cluster_cell_hitsTruthTotalE"]
    )
    ...
    unique_cell_IDs, unique_indices, cell_ID_counts = np.unique(
        cell_IDs_with_multiples_np, return_index=True, return_counts=True
    )
    ...
    for cell_id in enumerate(unique_cell_IDs[cell_ID_counts > 1]):
        unique_cell_Es[unique_cell_IDs == cell_id] += np.sum(
            cell_Es_with_multiples_np[cell_IDs_with_multiples_np == cell_id]
        )
    ...
    cell_hitsTruthPDGIDs = [
        [truthPDGID_arr[index] for index in cell]
        for cell in unique_cell_hitsTruthIndices
    ]
    cell_hitsTruthCharge = [
        [charge(pdgId) for pdgId in cell] for cell in cell_hitsTruthPDGIDs
    ]
...
def process_associated_cell_info(
    ...
    for cell_idx in range(len(filtered_cells)):
        ...
        if len(array_of_total_E) == 0:
            ...
            total_energy = array_of_total_E[
                0
            ]  # 0 index does not matter, so long as an index is chosen, all are same from broadcast at zip
        tracks_sample.field("Total_Truth_Energy").real(total_energy)

        focal_E = 0
        non_focal_E = 0
        nuetral_energy = 0
        for cell_part_ID, cell_truth_part_E, cell_truth_part_charge in zip(
            cell_part_IDs, cell_part_Es, cell_part_charges
        ):
        ...
        if total_energy > 0:
            ...
            frac_non_focal_E = 0
            frac_nuetral_energy = (
                1  # tracks with no truth cell E are assumed to be neutral
            )
...
def process_associated_tracks(
    ...
    for adj_track_idx in range(nTrack):
        ...
        if delta_r_adj <= MAX_DISTANCE:
            ...
            chi2_dof = (
                event["trackChiSquared"][adj_track_idx]
                / event["trackNumberDOF"][adj_track_idx]
            )
            ...
            for layer, (x, y, z, eta, phi) in adj_track_intersections.items():
                ...
                tracks_sample.field("delta_R_adj").real(
                    delta_r_adj
                )  # same for all layers
...
def train_val_test_split_events(
    ...
    for event_ids, fn in zip([train_ids, val_ids, test_ids], ["train", "val", "test"]):
        ...
        np.savetxt(
            AWK_SAVE_LOC(AWK).parent / f"{fn}_events_{split_seed=}.txt",
            event_ids,
            fmt="%d",
        )
...
def get_split(split_seed):
    ...
    for split in ["train", "val", "test"]:
        ids.append(
            np.loadtxt(
                AWK_SAVE_LOC(AWK).parent.parent
                / "AwkwardArrs"
                / f"{split}_events_{split_seed=}.txt"
            )
        )
    return ids
