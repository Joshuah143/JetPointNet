...
def split_and_save_to_disk(
    processed_data, base_filename, id_splits: dict, save_locations: dict
):
    """
    ...
    for idx, event_idx in enumerate(processed_data.eventNumber):
        ...
        if np.isin(event_idx[0], id_splits["train_ids"]):
            train_mask[idx] = True
            id_splits["train_ids"] = id_splits["train_ids"][
                id_splits["train_ids"] != event_idx[0]
            ]
        elif np.isin(event_idx[0], id_splits["val_ids"]):
            val_mask[idx] = True
            id_splits["val_ids"] = id_splits["val_ids"][
                id_splits["val_ids"] != event_idx[0]
            ]
        else:
            test_mask[idx] = True
            id_splits["test_ids"] = id_splits["test_ids"][
                id_splits["test_ids"] != event_idx[0]
            ]

    for mask, folder, split in zip(
        [train_mask, val_mask, test_mask],
        [
            save_locations["train_dir"],
            save_locations["val_dir"],
            save_locations["test_dir"],
        ],
        ["train", "val", "test"],
    ):
        if not np.any(mask):
        ...
        ak.to_parquet(
            data,
            os.path.join(folder, base_filename.split("/")[-1] + f"_{split}.parquet"),
        )
...
def process_events(
    ...
    for event_idx, event in enumerate(data):
        ...
        for track_idx in range(event["nTrack"]):
            ...
            focal_points = [
                (x, y, z)
                for _, (x, y, z, eta, phi) in focal_track_intersections.items()
            ]

            if len(focal_points) == 0:
                print(
                    f"Track {track_idx} of event {event['eventNumber']} skipped due to no track hits"
                )
                continue
...
def process_chunk(
    chunk, cell_ID_geo, cell_eta_geo, cell_phi_geo, cell_rPerp_geo, cell_Sigma_geo
):
    """
...
def event_handler_wrapper(filepath):
    ...
    if not OVERWRITE_AWK:
        print(
            f"Testing for existence of: {os.path.join(save_locations['train_dir'], existence_test_file)}"
        )

    # early return if the file already exists
    if not OVERWRITE_AWK and os.path.exists(
        os.path.join(save_locations["train_dir"], existence_test_file)
    ):
    ...
    try:
        with uproot.open(filepath + ":EventTree") as events:
            id_split = split_data(events, split_seed=62, retrieve=False)
            ...
            for chunk in events.iterate(
                ...
                processed_data = process_chunk(
                    chunk, cell_ID_geo, eta_geo, phi_geo, rPerp_geo, Sigma_geo
                )
                base_filename = f"{filepath}_chunk_{chunk_counter}"
                split_and_save_to_disk(
                    processed_data, base_filename, id_split, save_locations
                )
...
def main():
    ...
    if os.path.isfile(ROOT_FILES_DIR):
        ...
        potential_root_files = glob.glob(
            os.path.join(ROOT_FILES_DIR, "**/*.root"), recursive=True
        )
