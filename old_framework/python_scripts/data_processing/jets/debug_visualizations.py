...
def render_tracks(root_file, NAME="full_plots"):
    for event_index in range(len(root_file["EventTree"]["nTrack"].array())):
        ...
        if root_file["EventTree"]["nTrack"].array()[event_index] > 0:
            ...
            fig.suptitle(
                f"Event: {root_file['EventTree']['eventNumber'].array()[event_index]}"
            )
            ...
            track_to_index = root_file["EventTree"]["trackTruthParticleIndex"].array()[
                event_index
            ]
            ...
            for i in range(root_file["EventTree"]["nTrack"].array()[event_index]):
                ...
                for loc, rPerp in fixed_r.items():
                    eta = root_file["EventTree"][f"trackEta_{loc}"].array()[
                        event_index
                    ][i]
                    phi = root_file["EventTree"][f"trackPhi_{loc}"].array()[
                        event_index
                    ][i]
                    if phi > -500:
                ...
                name = (
                    Particle.from_pdgid(TruthPdgID[truth_index]).name
                    if truth_index != -1
                    else "??"
                )
                for ax in axl:
                    ax.plot(
                        ...
                        yl,
                        zl,
                        label=f"{name} | pt={pt_arr[i]:.2f} | ID={truth_index}",
                    )
                next_color = (
                    "red"
                    if truth_index == -1
            ...
            # plot truth data
            for truth_index in range(
                root_file["EventTree;1"]["nTruthPart"].array()[event_index]
            ):
                ...
                ax5.axvline(
                    truth_part_eta[truth_index],
                    label=f"TRUTH {name} | pt={truth_pt[truth_index]:.2f} | E={truth_E[truth_index]:.2f} | ID={truth_index}",
                    color=next_color,
                )
                ax6.axvline(
                    truth_part_phi[truth_index],
                    label=f"TRUTH {name} | pt={truth_pt[truth_index]:.2f} | E={truth_E[truth_index]:.2f} | ID={truth_index}",
                    color=next_color,
                )
            ...
            TruthIndex = root_file["EventTree"]["cluster_cell_hitsTruthIndex"].array()[
                event_index
            ]
            ...
            # Add Cell Hits
            for idx, cluster_ids in enumerate(
                root_file["EventTree"]["cluster_cell_ID"].array()[event_index]
            ):
                for cell_idx, cell_id in enumerate(cluster_ids):
                    try:
                        ...
                        if truth_index in energy_dep_pid.keys():
                            ...
                            energy_dep_pid[truth_index]["name"] = Particle.from_pdgid(
                                pdgid
                            ).name
            ...
            for pid, loc_dict in energy_dep_pid.items():
                for ax in axl:
                    ax.scatter(
                        ...
                        loc_dict["y"],
                        loc_dict["z"],
                        label=f"{loc_dict['name']} | ID={pid}",
                        s=2,
                    )

            eta_dict = {
                f"{loc_dict['name']} | ID={pid}": loc_dict["eta"]
                for pid, loc_dict in energy_dep_pid.items()
            }
            ...
            phi_dict = {
                f"{loc_dict['name']} | ID={pid}": loc_dict["phi"]
                for pid, loc_dict in energy_dep_pid.items()
            }
            ax6.hist(
                phi_dict.values(),
            ...
            plt.tight_layout()
            plt.savefig(
                f"root_images/{NAME}/en={root_file['EventTree']['eventNumber'].array()[event_index]}.png",
                dpi=800,
            )
...
def viz_awk(ak_array, focal_event_index=0, focal_track_index=0):
    ...
    # add focal track
    ax.plot(
        list(window["track_layer_intersections"]["X"]),
        list(window["track_layer_intersections"]["Y"]),
        list(window["track_layer_intersections"]["Z"]),
    )

    # add adj tracks
    for adj_track in window["associated_tracks"]:
        ax.plot(
            list(adj_track["track_layer_intersections"]["X"]),
            list(adj_track["track_layer_intersections"]["Y"]),
            list(adj_track["track_layer_intersections"]["Z"]),
        )

    ax.scatter(
        list(window["associated_cells"]["X"]),
        list(window["associated_cells"]["Y"]),
        list(window["associated_cells"]["Z"]),
        s=1,
        marker=".",
    )
