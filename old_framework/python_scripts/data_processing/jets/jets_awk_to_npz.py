...
from data_processing.jets.npz_utils import build_input_array
...
def build_arrays(data_folder, chunk_file_path, npz_data_folder_path):
    ...
    if not OVERWRITE_NPZ:
        print(
            f"Testing for existence of {os.path.join(NPZ_SAVE_LOC(NPZ), data_folder, data_set_name, chunk_file_path.split('/')[-1] + '.npz')}"
        )
        if os.path.exists(
            os.path.join(
                NPZ_SAVE_LOC(NPZ),
    ...
    feats, cell_truths = build_input_array(ak_array, global_max_sample_length)
...
def main():
    ...
    for data_folder in DATA_FOLDERS:
        ...
        chunk_files = [
            ...
            and any(f.split("/")[-1].startswith(i) for i in NPZ_ALLOWED_PREFIXES)
            ...
            results = list(
                tqdm(
                    pool.imap_unordered(
                        ...
                        zip(
                            [data_folder] * num_chunks,
                            sorted(chunk_files),
                            [npz_data_folder_path] * num_chunks,
                        ),
                    ),
                    total=num_chunks,
                )
