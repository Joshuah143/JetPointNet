...
def max_length_calculator_wrapper(full_path):
    ...
    (
        max_sample_length_arr,
        number_of_cells_in_track_arr,
        number_of_adj_track_arr,
        number_non_neg_adj_tracks,
    ) = calculate_max_sample_length_simplified(ak_array)
    return (
        max_sample_length_arr,
        full_path.split("/")[-1],
        number_of_cells_in_track_arr,
        ...
        for folder in tqdm(DATA_FOLDERS, desc="split loop"):
            ...
            for data_set in data_sets:
                ...
                folder_path = os.path.join(
                    AWK_SAVE_LOC(LEN), folder, data_set, "*.parquet"
                )
                files = [
                    file
                    for file in glob.glob(folder_path, recursive=True)
                    if file.endswith(".parquet") and os.path.isfile(file)
                ]
                files = files[:n_files_per_set]
                print(files)
                results[folder].extend(
                    list(
                        tqdm(
                            pool.imap(max_length_calculator_wrapper, files),
                            total=len(files),
