    ...
    *,  # ensures that all arguments must be named
    ...
    track_points.append(  # NOTE: Must match the dtype for the npz array
        (
            ...
            (
                normalized_cell_E
                if category == POINT_TYPE_ENCODING["cell"]
                else normalized_track_pt
            ),
        )
    )
...
def add_multitrack_truths(
    ...
    for truthIdx, relitive_E in zip(
        cell_Hits_TruthIndices, cell_Hits_TruthRelitiveEnergy
    ):
...
def build_input_array(tracks_sample_array, max_sample_length):
    ...
    for event in tracks_sample_array:
        for sample in event:
            ...
            if (
                sample["track_part_Idx"] == NPZ_PAD_VAL
                or NPZ_PAD_VAL in sample["associated_tracks"]["track_part_Idx"]
            ):
            ...
            for intersection in sample["track_layer_intersections"]:
                ...
                add_train_label_record(
                    ...
                    normalized_track_pt=sample["trackPt"]
                    / sample["total_sample_track_pt"],
            ...
            for track_idx, associated_track in enumerate(
                sorted(
                    sample["associated_tracks"],
                    key=lambda adj_track: adj_track["trackPt"],
                )
            ):
                ...
                for intersection in associated_track["track_layer_intersections"]:
                    ...
                    add_train_label_record(
                        ...
                        track_chi2_dof=associated_track[
                            "trackChiSquared/trackNumberDOF"
                        ],
                        cell_sigma=NPZ_PAD_VAL,
                        track_pt=associated_track["trackPt"],
                        normalized_track_pt=associated_track["trackPt"]
                        / sample["total_sample_track_pt"],
                        cell_E=NPZ_PAD_VAL,
                        normalized_cell_E=NPZ_PAD_VAL,
                        cell_ID=NPZ_PAD_VAL,
            ...
            for cell in sorted(
                sample["associated_cells"],
                key=lambda cell: calculate_delta_r(
                    sample["trackEta"], sample["trackPhi"], cell["eta"], cell["phi"]
                ),
            ):
                ...
                add_train_label_record(
                    ...
                    truth_cell_non_focal_fraction_energy=cell[
                        "Non_Focal_Fraction_Label"
                    ],
                    ...
                    cell_E=cell["E"],
                    normalized_cell_E=cell["E"]
                    / sample["total_associated_cell_energy"],
                    cell_ID=cell["ID"],
                    track_num=NPZ_PAD_VAL,
                )
                add_multitrack_truths(
                    cells_list,
                    track_partID_to_idx,
                    cell["cell_Hits_TruthIndices"],
                    cell["cell_Hits_TruthEs"] / cell["E"],
                )
